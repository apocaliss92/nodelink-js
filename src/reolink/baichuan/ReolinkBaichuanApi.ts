...
import {
  ...
  BC_CMD_ID_REPLAY_BY_TIME_V2,
} from "../../protocol/constants";
import { buildAbilityInfoExtensionXml, buildBinaryExtensionXml, buildChannelExtensionXml, buildFindEventLogGetXml, buildFindEventLogOpenXml, buildFloodlightManualXml, buildPreviewStopXml, buildPreviewStopXmlV11, buildPreviewXml, buildPreviewXmlV11, buildPtzControlXml, buildPtzPresetXml, buildPtzPresetXmlV2, buildReplayByTimeV2Xml, buildSirenManualXml, buildSirenTimesXml, buildStartZoomFocusXml, getXmlText, type ReplayByTimeV2StreamType, xmlEscape } from "../../protocol/xml";
import type {
  AIEvent,
  AIState,
  BatteryInfo,
...
<second>${endTime.getSeconds()}</second>
  ...
  // --------------------
  // HomeHub/NVR Event Log APIs
  // --------------------

  private parseEventLogTimeBlock(xml: string, blockTag: "startTime" | "endTime"): Date | undefined {
    const re = new RegExp(`<${blockTag}>\\s*<year>(\\d+)</year>\\s*<month>(\\d+)</month>\\s*<day>(\\d+)</day>\\s*<hour>(\\d+)</hour>\\s*<minute>(\\d+)</minute>\\s*<second>(\\d+)</second>\\s*</${blockTag}>`);
    const m = re.exec(xml);
    if (!m) return undefined;
  ...
  private parseEventLogInfo(xml: string): { handle?: number; finished?: boolean; events: Array<Omit<ReolinkEventLogEntry, "channelId">> } {
    const handle = (() => {
      const v = getXmlText(xml, "handle");
      const n = v != null ? Number(v) : NaN;
      return Number.isFinite(n) ? n : undefined;
    })();
    const finished = (() => {
      const v = getXmlText(xml, "bFinished");
      if (v == null) return undefined;
      const n = Number(v);
    ...
    const eventBlocks = Array.from(xml.matchAll(/<eventLog>([\s\S]*?)<\/eventLog>/g))
      .map((m) => m[1])
      .filter((b): b is string => typeof b === "string");
    const events: Array<Omit<ReolinkEventLogEntry, "channelId">> = [];
    for (const b of eventBlocks) {
      const uid = (getXmlText(b, "uid") ?? "").trim();
      const logicChnRaw = getXmlText(b, "logicChn");
      const logicChn = logicChnRaw != null && logicChnRaw.trim() !== "" ? Number(logicChnRaw) : undefined;
      const logicChnBitmap = Number.isFinite(logicChn as number) ? (1 << (logicChn as number)) : undefined;

      const alarmType = (getXmlText(b, "alarmType") ?? "").trim() || undefined;
      const startTime = this.parseEventLogTimeBlock(b, "startTime");
      const endTime = this.parseEventLogTimeBlock(b, "endTime");
      if (!uid || !startTime || !endTime) continue;

      const hasRecFile = asBool01(getXmlText(b, "bHasRecFile"));
      const encrypted = asBool01(getXmlText(b, "bEncrypted"));
      const deleted = asBool01(getXmlText(b, "bDeleted"));

      events.push({
        uid,
        ...(Number.isFinite(logicChn as number) ? { logicChn: logicChn as number } : {}),
        ...(logicChnBitmap !== undefined ? { logicChnBitmap } : {}),
        ...(alarmType ? { alarmType } : {}),
        startTime,
        endTime,
    ...
    return { ...(handle !== undefined ? { handle } : {}), ...(finished !== undefined ? { finished } : {}), events };
  }

  /**
   * List HomeHub/NVR events between start/end.
   *
   * Note: HomeHub PCAP shows cmd_id 516 opens a cursor and cmd_id 517 pages results.
   * The "startTime" passed to open appears to behave like an upper bound (descending order).
   *
   * This method does NOT require the caller to pass eventAlarmType/devices.
   * It will automatically include all known device UIDs from cmd_id 145 push cache.
   */
  async listEventLogs(params: {
    start: Date;
    end: Date;
    /** Max number of events to return. If omitted, returns all events within the window (best-effort). */
    count?: number;
    /** Max events to request per page (default: 60, matches PCAP). */
    pageSize?: number;
    /** Optional Hub channelId filter (0-based). Applied client-side after parsing. */
    channelId?: number;
    /** If true, only returns entries with hasRecFile===true. Applied client-side after parsing. */
    requireHasRecFile?: boolean;
  }): Promise<ReolinkEventLogEntry[]> {
    await this.client.login();
    ...
    if (!(start instanceof Date) || isNaN(start.getTime())) throw new Error("listEventLogs: invalid start Date");
    if (!(end instanceof Date) || isNaN(end.getTime())) throw new Error("listEventLogs: invalid end Date");

    const pageSize = Math.max(1, Math.min(200, Math.trunc(params.pageSize ?? 60)));

    const push = this.getChannelInfoFromPushCache();
    const uidToChannel = new Map<string, number>();
    for (const [ch, info] of push.entries()) {
      const uid = (info.uid ?? "").trim();
      if (!uid) continue;
      if (!uidToChannel.has(uid)) uidToChannel.set(uid, ch);
    }
    const deviceUids = Array.from(uidToChannel.keys());
    if (deviceUids.length === 0) {
      throw new Error(
        "listEventLogs: no device UIDs available. On HomeHub/NVR this typically requires cmd_id 145 push cache (connect/login first) or a provided UID mapping.",
      );
    }

    // Open cursor (516). Observed to return a handle.
    // PCAP includes alarmType/eventAlarmType; some firmwares return empty results if omitted.
    // We keep this internal (callers don't need to pass filters) and use a broad allow-list.
    const alarmTypes = [
      "md",
      "pir",
      "io",
    ...
    const openXml = buildFindEventLogOpenXml({
      // In PCAP, this behaves like an upper bound with desc=1.
      startTime: end,
      deviceUids,
      alarmTypes,
      logicChnBitmap: 255,
    ...
    const openRsp = await this.sendXml({ cmdId: BC_CMD_ID_FIND_EVENT_LOG_OPEN, payloadXml: openXml });
    const handleText = getXmlText(openRsp, "handle");
    const handle = handleText != null ? Number(handleText) : NaN;
    if (!Number.isFinite(handle)) {
      throw new Error(`listEventLogs: failed to open event log cursor (missing handle). response=${openRsp.slice(0, 200)}`);
    }

    const out: ReolinkEventLogEntry[] = [];
    let finished = false;
    let safety = 0;
    ...
    const channelIdFilter =
      params.channelId === undefined
        ? undefined
        : (() => {
            const ch = Math.trunc(params.channelId as number);
            return Number.isFinite(ch) && ch >= 0 ? ch : undefined;
          })();
    const requireHasRecFile = params.requireHasRecFile === true;

    while (!finished && out.length < countLimit) {
      // Hard cap to avoid infinite loops when the device lies about bFinished.
      // With default pageSize=60, 250 pages is 15k events.
      if (safety++ > 250) break;
      const remaining = Number.isFinite(countLimit) ? Math.max(0, countLimit - out.length) : pageSize;
      const maxEventCount = Math.min(pageSize, remaining);
      const getXml = buildFindEventLogGetXml({ handle, maxEventCount });
      const pageRsp = await this.sendXml({ cmdId: BC_CMD_ID_FIND_EVENT_LOG_GET, payloadXml: getXml });
      const parsed = this.parseEventLogInfo(pageRsp);

      if (parsed.finished !== undefined) finished = parsed.finished;

      // Map uid -> hub channelId (best-effort) and apply start/end filter.
      for (const e of parsed.events) {
        if (e.endTime.getTime() < start.getTime()) {
          // Since results are descending, once we're older than start we can stop.
          finished = true;
          break;
        }
        if (e.startTime.getTime() > end.getTime()) {
          // Newer than requested window; skip.
          continue;
        }

        const channelId = uidToChannel.get(e.uid);
        const full: ReolinkEventLogEntry = {
          ...e,
          ...(channelId !== undefined ? { channelId } : {}),
        };

        if (requireHasRecFile && full.hasRecFile !== true) continue;
        if (channelIdFilter !== undefined && full.channelId !== channelIdFilter) continue;

        out.push(full);
        if (out.length >= countLimit) break;
      }

      // If the device returned nothing, avoid infinite loop.
      if (parsed.events.length === 0 && parsed.finished === undefined) break;
    }
  ...
  /**
   * Send ReplayByTimeV2 (cmd_id=381) for a single event duration.
   * This primes the device to start streaming the selected clip over the native Baichuan stream.
   */
  async startEventPlayback(params: {
    uid: string;
    /** Hub channelId (0-based) that corresponds to the uid. */
    channelId: number;
    startTime: Date;
    endTime: Date;
    /** Logical channel bitmap (typically 1). Defaults to 1. */
    logicChnBitmap?: number;
    /** Prefer mobileStream over subStream by default. */
    streamType?: ReolinkEventReplayStreamType;
  }): Promise<{ streamType: ReolinkEventReplayStreamType; responseXml: string }>
  {
    await this.client.login();

    const uid = params.uid.trim();
    if (!uid) throw new Error("startEventPlayback: uid is required");
    const channelId = Math.trunc(params.channelId);
    if (!Number.isFinite(channelId) || channelId < 0) throw new Error(`startEventPlayback: invalid channelId: ${params.channelId}`);
    const startTime = params.startTime;
    const endTime = params.endTime;
    if (!(startTime instanceof Date) || isNaN(startTime.getTime())) throw new Error("startEventPlayback: invalid startTime");
    if (!(endTime instanceof Date) || isNaN(endTime.getTime())) throw new Error("startEventPlayback: invalid endTime");
    if (endTime.getTime() <= startTime.getTime()) throw new Error("startEventPlayback: endTime must be > startTime");

    const streamType: ReolinkEventReplayStreamType = params.streamType ?? "mobileStream";
    const logicChnBitmap = Math.trunc(params.logicChnBitmap ?? 1);

    const seq = Math.floor(Date.now() / 1000);
    const payloadXml = buildReplayByTimeV2Xml({
      seq,
      startTime,
      // PCAP uses day-end; builder defaults to 23:59:59.
      streamType: streamType as ReplayByTimeV2StreamType,
      duration: {
        channelId,
        uid,
        logicChnBitmap,
    ...
    const responseXml = await this.sendXml({ cmdId: BC_CMD_ID_REPLAY_BY_TIME_V2, payloadXml });
    return { streamType, responseXml };
  }
   ...
   * when the first client connects (matching PCAP behavior: replay after preview starts).
   */
  async createEventPlaybackRtspUrl(params: {
    event: ReolinkEventLogEntry;
    /** Prefer mobileStream > subStream (default). */
    preferredStream?: ReolinkEventReplayStreamType;
    listenHost?: string;
    listenPort?: number;
    /** Optional RTSP path; default is derived from uid + timestamps. */
    path?: string;
  }): Promise<{ url: string; streamType: ReolinkEventReplayStreamType; stop: () => Promise<void> }> {
    const ev = params.event;
    const uid = (ev.uid ?? "").trim();
    const channelId = ev.channelId;
    if (!uid) throw new Error("createEventPlaybackRtspUrl: event.uid is required");
    if (channelId === undefined) {
      throw new Error(
        "createEventPlaybackRtspUrl: event.channelId is missing. Ensure events were produced by listEventLogs() (which resolves uid->channelId via cmd_id 145 push cache).",
      );
    }
    ...
    const safeUid = uid.replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 24);
    const safeTs = `${Math.floor(startTime.getTime() / 1000)}-${Math.floor(endTime.getTime() / 1000)}`;
    const path = params.path ?? `/event-${safeUid}-${safeTs}`;

    // Heuristic mapping for "mobileStream" preference: try telephoto variant on sub.
    // If this doesn't match the target firmware, we'll adjust during integration tests.
    const variant: NativeVideoStreamVariant | undefined = streamType === "mobileStream" ? "telephoto" : "default";
    ...
    const server = new BaichuanRtspServer({
      ...
      profile,
      ...(variant ? { variant } : {}),
      ...(params.listenHost ? { listenHost: params.listenHost } : {}),
      ...(params.listenPort ? { listenPort: params.listenPort } : {}),
      path,
      logger: this.logger,
    });
    ...
    const trigger = async (): Promise<void> => {
      ...
      try {
        ...
        this.logger.warn?.("[ReolinkBaichuanApi] startEventPlayback failed", e);
      }
    };
